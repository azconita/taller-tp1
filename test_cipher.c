...
void test_cipher_enc_dec(const char *filename);

int main(int argc, char const *argv[]) {
  test_cipher_enc_dec(argv[1]);
  return 0;
}
...
void test_cipher_enc_dec(const char *filename) {
  unsigned char buf[64], encrypted[64], decrypted[64];
  cipher_t cipher_e, cipher_d;
  int keylength, readed;
  char key[8] = "Key";
  keylength = 3;
  cipher_init(&cipher_e, key, keylength);
  cipher_init(&cipher_d, key, keylength);
  readed = read_input_file(filename, buf);
  printf("original text:\n");
  print_hexa(buf, readed);
  cipher_encrypt(&cipher_e, buf, encrypted, readed);
  printf("encrypt:\n");
  print_hexa(encrypted, readed);
  cipher_decrypt(&cipher_d, encrypted, decrypted, readed);
  printf("decrypt:\n");
  print_hexa(decrypted, readed);
}
